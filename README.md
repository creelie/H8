...
| `closure_graph.py` | the logical skeleton of the paper and of the literature it quotes as a rule set: the conjecture is not in the closure of what is proved; there are exactly four minimal sufficient sets, {Lefschetz standard conjecture B, every Hodge class motivated}, {B, (F3)}, {variational Hodge conjecture for algebraic classes, (F3)} and the route of the paper {(P2), (F2), (F3)}; no single open statement suffices; every member of these sets except (P2) is itself implied by the conjecture; and the secant route lies in no minimal set |
...
| `mumford_routes.py` | which of the open routes to the Mumford target are needed: sixteen statements and twenty Horn rules, each labelled by the theorem that proves it and checked against paper_labels.txt; the target is not in the closure of what is proved; the statements equivalent to it are exactly algebraicity at uncountably many points, B(W x_C W), bounded data at infinitely many points and bounded data modulo p; every minimal set of open statements yielding it has one element; the complex with dim Ext^2 = 119 and the Kuga-Satake statements are stronger than it; (L) and (V) yield it; and it yields neither the Hodge conjecture nor (F1), (F2), (F3) |
...
## Rebuilding the figures
    ...
    python3 make_frontier.py    # the four minimal sufficient sets as ribbons,
...
abelian, and from no two of the three (`closure_graph.py`, and Section 24 of
the Lean file). The last two are not reductions: the self-product of a
...
With the implications of the literature added, the rule set has four minimal
sufficient sets: the Lefschetz standard conjecture (L) with the motivatedness
of every Hodge class (M), which together are equivalent to the conjecture;
(L), or the variational Hodge conjecture for algebraic classes (V), with the
conjecture for varieties that are not abelian; and the three statements
above. Every member except the propagation statement is a consequence of the
conjecture (`closure_graph.py`, Section 24 of the Lean file). One new case of
...
cases of (F2) and (F3) open (`mumford_routes.py`).
