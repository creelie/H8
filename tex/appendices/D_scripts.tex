...
line \texttt{823 checks passed, 0 failed}. Everything requires the standard library of Python~3,
...
\texttt{numpy} for \texttt{pte\_remaining.py}, where it
holds integer arrays; no step uses floating point.
...
\begin{verbatim}
  [PASS] the minimal sufficient sets are enumerated exhaustively
         4 of them, over the 12 open statements, from 4096 subsets
  [PASS] the minimal sufficient sets are exactly four
         {lef_B, mot}, {lef_B, red_ab}, {red_ab, vhc} and {P2, red_ab,
         red_weil}
  [PASS] no single open statement suffices
         the smallest sufficient sets have 2 elements, and there are 3 of
         them
  [PASS] the route through this paper is the only minimal sufficient set
       ...
       the minimal sufficient sets:
            ...
            {P2, red_ab, red_weil}
       with
            P2         the semiregularity map of a complex with the Weil
                       Chern character at a base point is injective, in
                       every Weil family of every CM field; it forces the
                       evaluation map to vanish on the annihilator of the
                       Weil class
            lef_B      the Lefschetz standard conjecture B(X) for every
                       smooth complex projective variety X
            mot        every Hodge class on every smooth complex
                       projective variety is motivated in the sense of
                       Andre
            red_ab     the Hodge conjecture for every smooth projective
                       variety that is not an abelian variety; not a
                       reduction, since the H^3 of a very general quintic
                       threefold is not of abelian type
            red_weil   every Hodge class on an abelian variety outside the
                       subring generated by divisor classes and Weil
                       classes of quotients is algebraic; not a reduction,
                       since the self-product of a Mumford fourfold
                       carries two such classes
            vhc        a global section of R^{2p} f_* Q for a smooth
                       projective family over a smooth connected base that
                       is algebraic at one point is algebraic at every
                       point

  [PASS] every member of a minimal sufficient set other than P2 is a
  ...
  [PASS] the route {lef_B, mot} is an equivalence with the conjecture
         it suffices, and both members follow from the conjecture
  [PASS] every minimal sufficient set contains red_ab or mot
         the varieties that are not abelian are reached only through the
         conjecture for them or through motivated classes
  [PASS] the variational statement alone gives the conjecture for abelian
  ...
  [PASS] the Lefschetz standard conjecture gives the variational statement
  ...
  [PASS] the secant route lies in no minimal sufficient set
         ...
         the conjecture
  [PASS] the secant route, fully granted, reaches the trivial discriminant
         and stops
         every base point it produces is a split member, and a split
         member has trivial discriminant
  [PASS] the trivial discriminant alone does not close the imaginary
         quadratic case
         the families of nontrivial discriminant are a separate problem
  [PASS] the propagation statement closes the imaginary quadratic case
         outright
         a base point exists in every family, so the propagation is
         uniform in K, in n and in the discriminant
  [PASS] the propagation statement closes the Weil classes of every CM
         field outright
  ...
  [PASS] the Weil classes of the higher CM fields are derived, not open
       ...
       the conjecture from the proved and quoted statements and lef_B, mot
            HC            from lef_B, mot
                                                             [prop:lefmot]
       ...
       the conjecture from the proved and quoted statements and P2,
         red_ab, red_weil
            weil_triv     from base_point, reduction, orbit_dense, P2
                                                      [thm:semiregclosure]
            weil_nontriv  from base_point, reduction, orbit_dense, P2
                                                      [thm:semiregclosure]
            weil_cm       from base_point_cm, reduction_cm,
                               orbit_dense_cm, P2
                                                       [thm:cmpropagation]
...
\begin{verbatim}
  ...
  [PASS] every minimal set of open statements yielding the target has
         ...
         11 minimal sets: BOUND, B_WW, EXT119, F2, HC, HK_U, KS_U, L,
         MODP, UNCOUNT, V
