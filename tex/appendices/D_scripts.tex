...
line \texttt{951 checks passed, 0 failed}. Everything requires the standard library of Python~3,
...
\texttt{numpy} for \texttt{pte\_remaining.py}, \texttt{weil\_tori.py} and
some of the scripts that item (LV) runs, where it holds integer arrays and
reduces them modulo a prime, and \texttt{python-flint} for item (LV), where its
\texttt{fmpq\_mat} computes exact ranks over $\QQ$; no step uses floating
point.
