...
several of them, \texttt{numpy} for integer arrays in a few and
\texttt{python-flint} for exact rational matrices in item (LV); they work over $\QQ$ and
...
\item \emph{The split member.} The script \texttt{split\_locus.py} builds
  ...
  Third it checks the formulas of \Cref{ex:splitsmall} as they are printed,
  at $n=2$ for $d\le3$ and at $n=3$ for $d\in\{1,2,3,7\}$; this check was
  added after an earlier printing of $\omega_{1}$ at $n=3$ was found to lack
  the factor $d$, and it fails on that printing for $d\ne1$.
...
\item \emph{The closure graph.} The script \texttt{closure\_\allowbreak graph.py}
  carries the rule set of \Cref{setup:rules} as data: forty-two statements,
  each marked proved here, quoted, open or derived, and twenty-two rules, each carrying
  ...
  rule set is acyclic, and that every one of the seventeen theorem labels it
  names, for its rules and for the consequences of $\mathbf{HC}$ it records,
  is a label of this paper, against the list of all $546$ labels generated
  ...
  and, by exhaustive search over all $2^{16}$ subsets of the sixteen open
  ...
  families of the CM fields of degree at least four, that (F3) is the one open
  ...
  $\{\mathrm{P2}_{\mathrm{split}},\mathrm{F2},\mathrm{F3}\}$, $\{\mathrm{L},\mathrm{F3}\}$
  ...
  than $\mathrm{P2}_{\mathrm{split}}$ is recorded as a consequence of $\mathbf{HC}$, and that
  ...
  that the trivial discriminant closes the imaginary quadratic case and the
  split families of the fields of degree at least four close it as well; that
  the propagation statement for the split imaginary quadratic families
  reaches all of that case, and for the split families of the higher fields
  the Weil classes of every CM field, so that those are not open; that no
  minimal set contains a propagation statement for a family of nontrivial
  discriminant or for an imaginary quadratic field; and that the bounded criterion
  ...
  $\{\mathrm{V},\mathrm{F3}\}$ and $\{\mathrm{P2}_{\mathrm{split}},\mathrm{F2},\mathrm{F3}\}$;
...
\item \emph{The annihilator of a Weil class in Hochschild cohomology.} The
  ...
  in every degree $k\ge1$ (and $0$ in degree $0$), that the ideal generated by $P\wedge Q$ consists of the
...
\item \emph{The Hodge classes of a very general Weil torus.} The script
  ...
  $\alpha_{+}\wedge\kappa^{n}=\alpha_{-}\wedge\kappa^{n}=0$, and
  $\alpha_{+}\wedge(\kappa+\beta)^{n}=\alpha_{+}\wedge\beta^{n}$ becomes
  nonzero once a component $\beta\in\bigwedge^{2}V_{-}$ with $\beta^{n}\ne0$
  is added (while $\alpha_{-}\wedge(\kappa+\beta)^{n}$ stays zero, every term
  having a factor from $V_{-}$; symmetrically for $\beta'\in\bigwedge^{2}V_{+}$
  and $\alpha_{-}$), so the vanishing used for the block diagonal K\"ahler
  class $\kappa_{0}$ is a property of $V_{+}\otimes V_{-}$ and not of the
  coordinates.

\item \emph{The corrected criterion as a number.} The script
  \texttt{p2prime.py} works in the eigenbasis model of item (XXXVIII), with
  ...
  computes ranks exactly over $\QQ(i)$ by blocks of torus weight, which is
  legitimate because contraction is equivariant for the torus of
  ...
  $\int\theta^{2n}=(2n)!$ for $n=2,3$, and in the rational model of
  \texttt{explicit\_weil.py} at $n=2$, $\int\omega_{1}^{2}=8d^{2}$ and
  ...
  classes, while at the ratio $R=3/16$ both signs occur. That is
  \Cref{prop:p2primelocus} and \Cref{rem:p2primeexceptional}. With
  \texttt{--extreme} the closed form
  of (A) is checked exactly for $n=5,\dots,9$ on five shapes, and with
  \texttt{--nmax=10} also at $n=10$, where $HT^{2}$ has dimension $780$ and
  $\gamma$ has $2^{20}+2$ terms; that run takes $142$ minutes and its
  transcript is in \texttt{code/extreme}. That is \Cref{thm:p2primenumber}.

\item \emph{The Hochschild profile of a corrected character.} The script
  \texttt{p2prime\_\allowbreak profile.py} computes, in the same model and by
  the same torus blocks, the rank $\rho_{k}$ of contraction into $\gamma$ on
  every $HT^{k}$, exactly over $\QQ(i)$. (A) For $n=2,3$, in every degree
  $0\le k\le4n$, on eight shapes and two values of $u$, the ranks equal
  ...
  point $|u|=4|c_{2}|$ of $\theta^{2}+\omega$ has $\rho_{2}=23$. With
  \texttt{--extreme} (A) is run for $n=4$ in every degree, where $HT^{8}$ has
  dimension $12870$, and for $n=5$ up to degree three. That is
  \Cref{prop:p2primeprofile}.

\item \emph{Descent and scalar extension.} The script \texttt{descent.py}
  models the cohomology of an abelian variety of $(F,n)$-Weil type, $F$ a CM
  field of degree $2m$, by alternating $\QQ$-multilinear forms on
  $H_{1}=F^{N}$, its Weil classes by the forms $\Tr_{F/\QQ}(a\det_{F})$ and its
  ...
  $j^{*}W_{F}(B_{F})$ is all of $W_{K}(B)$. With \texttt{--extreme}, (A) is
  run up to $n=5$ over $\QQ(i)$, up to $n=4$ over $\QQ(\sqrt{-2})$ and
  $\QQ(\sqrt{-3})$, up to $n=3$ over $\QQ(\sqrt{-5})$, $\QQ(\zeta_{5})$ and
  $\QQ(\zeta_{8})$, and up to $n=2$ over the sextic field $\QQ(\zeta_{7})$, and
  (C) up to $n=3$. That is \Cref{prop:descent}.
\end{enumerate}

Items (I), (II), (III), (VI), (VII), (VIII) and (X) to (LIV) are the ones
...
Running \texttt{verify\_all.py} executes the forty-nine Python items and
prints \texttt{951 checks passed, 0 failed}; items (XXIX), (XXXI) and
