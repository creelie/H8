...
Python, together with \texttt{sympy} for exact symbolic linear algebra in two
of them and \texttt{numpy} for integer arrays in one; they work over $\QQ$ and
...
\item \emph{The closure graph.} The script \texttt{closure\_\allowbreak graph.py}
  carries the rule set of \Cref{setup:rules} as data: thirty-six statements,
  each marked proved here, quoted, or open, and fifteen rules, each carrying
  ...
  rule set is acyclic, and that every one of the thirteen theorem labels it
  names, for its rules and for the consequences of $\mathbf{HC}$ it records,
  is a label of this paper, against the list of all $521$ labels generated
  ...
  and, by exhaustive search over all $2^{12}$ subsets of the twelve open
  statements, that there are exactly the four minimal sufficient sets of
  \Cref{thm:closure}(ii), that no single open statement suffices, that the
  route through this paper is the only one with three elements and the only
  one whose derivation uses a statement proved here, that each element of
  each set is necessary, that every set contains (F3) or \textup{(M)}, and
  that every element of every set other than (P2) is recorded as a
  consequence of $\mathbf{HC}$. It confirms that \textup{(V)} alone gives the
  ...
  is absent from the rule set, being equivalent to its own conclusion.
  ...
  \Cref{thm:closure} and \Cref{cor:fourremain}.
...
\item \emph{Which of the open routes to the Mumford target are needed.} The
  ...
  points), and twenty Horn rules, each labelled by the theorem that proves it;
  unlike that rule set this one contains equivalences. (A) Its ten labels are
  labels of the paper and its statements are all used. (B) The target is not
  ...
  that yield the target, found by exhaustive search, number eleven and each
  ...
  (F2), (F3), \textup{(L)}, \textup{(V)}, \textup{(M)}. That is
  \Cref{cor:mumfordone} and \Cref{rem:mumfordneeded}.
\end{enumerate}

Items (I), (II), (III), (VI), (VII), (VIII) and (X) to (XLIX) are the ones
...
Running \texttt{verify\_all.py} executes the forty-four Python items and
prints \texttt{823 checks passed, 0 failed}; items (XXIX), (XXXI) and
