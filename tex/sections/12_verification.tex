...
\item \emph{The closure graph.} The script \texttt{closure\_\allowbreak graph.py}
  ...
  each marked proved here, quoted, or open, and sixteen rules, each carrying
  ...
  rule set is acyclic, and that every one of the fourteen theorem labels it
  names, for its rules and for the consequences of $\mathbf{HC}$ it records,
  is a label of this paper, against the list of all $523$ labels generated
  ...
  statements, that there are exactly the two minimal sufficient sets of
  \Cref{thm:closure}(ii), $\{\mathrm{F3}\}$ and $\{\mathrm{L},\mathrm{M}\}$,
  that (F3) is the one open statement that suffices alone, that the route
  through this paper and the sets $\{\mathrm{L},\mathrm{F3}\}$ and
  $\{\mathrm{V},\mathrm{F3}\}$ suffice and are not minimal, that neither
  minimal set uses a statement proved here, that each element of each set
  is necessary, that every set contains (F3) or \textup{(M)}, and that every
  element of every set is recorded as a consequence of $\mathbf{HC}$, so that
  each set is equivalent to it. It confirms that \textup{(V)} alone gives the
  ...
  then removes the one rule of \Cref{prop:f3ishc} and repeats the search,
  which returns the four sets of the earlier version of this paper,
  $\{\mathrm{L},\mathrm{M}\}$, $\{\mathrm{L},\mathrm{F3}\}$,
  $\{\mathrm{V},\mathrm{F3}\}$ and $\{\mathrm{P2},\mathrm{F2},\mathrm{F3}\}$;
  so the change in the answer is that rule and nothing else.
  ...
  \Cref{thm:closure}, \Cref{prop:f3ishc} and \Cref{cor:fourremain}.
...
prints \texttt{827 checks passed, 0 failed}; items (XXIX), (XXXI) and
