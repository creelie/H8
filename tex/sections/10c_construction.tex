...
\begin{proof}
Let $\mathfrak{M}$ be the stack of perfect complexes on the fibres of
$f\colon\cA\to\cS$. It is algebraic and locally of finite presentation over
$\cS$ \cite{Lie06}, and at a point $[E]$ over $s_{0}$ its deformation theory
relative to $\cS$ has tangent space $\Ext^{1}(E,E)$ and obstruction space
$\Ext^{2}(E,E)$.
...
The obstruction disappears as soon as the Chern character is allowed a
component that is Hodge on the polarised family and not beyond it. Call
...
that subring is spanned by the powers of $\theta$ and the Weil plane. What is excluded is only a Chern character that remains of Hodge type
on all of $S$, and by \Cref{lem:weiltorushodge} such a character is a
multiple of the Weil class plus terms in degrees $0$ and $4n$. Off the
polarised family the components $c_{k}\theta^{k}$ with $c_{k}\ne0$ stop being
of Hodge type, so nothing forces $E$ onto non-algebraic tori, and the argument
of \Cref{thm:p2false} does not apply. The semiregular objects in Markman's
...
read in this corrected form. Nothing in this paper analyses it: the annihilator
computations above depend on $\ch(E)$ being a pure Weil class and have to be
redone for a Chern character with components in every even degree.
...
\Cref{rem:p2prime} left the corrected criterion unanalysed, because the
annihilator computations of this subsection depend on $\ch(E)$ being a pure
Weil class. We now redo them for a Chern character with components in every
...
\item $\rho=3$ for a general $p$, and then $\Ann_{HT^{2}}(\gamma)$ is exactly
...
\item If $p\in\CC\theta^{2n}$, or $p\in\CC e^{t\theta}$ with $t\theta$ the class
  of a line bundle $L$, then no perfect complex $E$ with $\ch(E)=\gamma$ has
  injective $\sigma_{E}$.
\end{enumerate}
\end{theorem}
...
$p=c\,\theta^{2n}$ then $\gamma-\omega$ has components in degrees $0$ and $4n$
only, and the proof of \Cref{thm:p2false} applies verbatim. If
$p=c\,e^{t\theta}$ with $t\theta=c_{1}(L)$, then
$\ch(E\otimes L^{-1})=\gamma e^{-t\theta}=c+\omega$ because $\omega\theta=0$,
and $\sigma_{E\otimes L^{-1}}=e^{-c_{1}(L)}\cdot\sigma_{E}$ is injective
exactly when $\sigma_{E}$ is, so \Cref{thm:p2false} applies to
$E\otimes L^{-1}$.
...
\item In \Cref{setup:p2prime} with $n\ge3$, the germ at $A$ of the Hodge locus
  ...
  otherwise. In the first case every member of the germ is an abelian variety
  polarised by $\theta$.
\item For $n=2$ the same holds, with one exception: if $c_{1}=c_{3}=0$ and
  ...
  the character $N\omega_{1}\pm\tfrac{d}{2}N\eta^{2}+c_{0}+c_{4}\eta^{4}$, so the
  exception occurs in every family.
\end{enumerate}
\end{proposition}
...
the symplectic form $y$ on a space of dimension $4n$, so near $\theta$ that set
...
polarised by $\theta$. (iv) At $n=2$ the weight argument of
...
and $\int\eta^{4}=24$, which gives $c_{2}=\pm dN/2$. Items (LII)(E) and (F)
of \Cref{sec:verification} check these dimensions and the two integrals
exactly.
\end{proof}
...
there is nothing for the argument through Weil tori to act on. The only
corrected characters it still reaches are those of
\Cref{thm:p2primenumber}(v), and, at $n=2$, possibly the exceptional ratio of
(iv), whose Hodge locus is five-dimensional and so leaves the polarised
family.
...
\Cref{lem:finiteproduct}; and \Cref{thm:p2numerical}(i)--(ii), which is
\Cref{thm:p2primenumber}(iv). Unchanged only when
...
basis, and nothing else. An object with no such summand, whose Chern
character is not effective, and with $\dim\Ext^{2}(E,E)=7n^{2}-2n$, is not
excluded by anything proved here, and none is known.
\end{remark}
