...
and at $n=3$,
\[
  \omega_{1}=d\,\gamma\,\ell^{2}-\tfrac13\gamma^{3},
  \qquad
  \omega_{2}=\gamma^{2}\ell-\tfrac13\,d\,\ell^{3},
\]
with $\gamma=d\beta-\widehat{\beta}$ throughout.\footnote{An earlier version
printed $\gamma\ell^{2}-\frac13\gamma^{3}$ for $\omega_{1}$ at $n=3$, which lies
in the Weil plane only when $d=1$; the displayed formulas are now checked as
printed, for several $d$, by item~(X).} Both cases, and the expansion
