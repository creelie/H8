...
construction of this section produces a base point in exactly one of them, the
one with trivial discriminant. So \Cref{q:residual} is really two questions,
...
\item \emph{Trivial discriminant.} A base point exists, with the explicit
  ...
  full Hodge group. For $n\ge3$ it is open.
...
\begin{theorem}[Buchweitz and Flenner \cite{BF03}]\label{thm:BF}
...
\item For every $k$,
  ...
  the annihilator is the ideal generated by its degree-two part together with
  that one line.
\end{enumerate}
\end{theorem}
...
$V_{-}^{1,0}$, with conjugate bases $\bbar x_{i}$, $\bbar y_{j}$ of
$V_{\pm}^{0,1}$, so that $\alpha_{+}=x_{1}\cdots x_{n}\bbar x_{1}\cdots\bbar x_{n}$
...
by the theorem of Buchweitz and Flenner, which holds for perfect complexes on
complex spaces \cite{BF03}, every obstruction to extending $E$ along an
...
algebraic, and $\theta^{n}$ is algebraic, so $\omega_{1}$ is. The same proof
allows any further components in the subring generated by $\theta$ and the
Weil classes, such as $\theta^{j}\omega_{1}$, since they stay of Hodge type
along the polarised family and do not enter $\ch_{n}$; we state the simplest
form. What is excluded is only a Chern character that remains of Hodge type
