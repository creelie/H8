...
semiregularity theorem of Buchweitz and Flenner \cite{BF03}, in the form
for perfect complexes on a deforming variety with every obstruction
annihilated that Pridham proves \cite{Pri24}, the theorem of
...
\item \emph{One finite sufficient condition, false in its first form, and
  ...
  would force (\Cref{thm:p2endo}) is the shape of nothing. In the corrected
  form, with powers of the polarisation added, the number becomes
  $(4+\rho)n^{2}-2n$ for $n\ge3$, with $\rho\le3$ the rank of a Hankel matrix
  of the coefficients; an object attaining it satisfies the criterion, and
  the shapes $A\,e^{t\theta}+B\,\theta^{2n}$ with $t\theta$ the class of a
  line bundle, of Hankel rank at most two, are excluded as the pure form
  was (\Cref{thm:p2primenumber}). For every other shape and $n\ge3$ the Hodge
  locus of the character is the polarised family, so the pure-form
  obstruction does not extend (\Cref{prop:p2primelocus}). The
...
\item \emph{Every CM field, not only the quadratic ones.} For every CM field
  ...
  (\Cref{prop:cmweil}, \Cref{prop:composite}). The families are linked as
  well: for each CM field the split families in infinitely many dimensions
  give all of its families, and the fields of degree at least four give the
  imaginary quadratic ones (\Cref{prop:descent}).
\item \emph{What is left, and that it is exactly that.} The Hodge conjecture
  ...
  statements, namely the propagation statement of (iv), read for the split
  families of the CM fields of degree at least four, which by
  \Cref{prop:descent} carry every imaginary quadratic family with them, the
  algebraicity of the Hodge classes
...
The base case is not the difficulty. By (iii) every family has a base point,
for every field, every dimension and every discriminant, and the cycle is a
polynomial in two divisor classes rather than an existence statement. Nor is
the discriminant: by \Cref{prop:descent} the split families in all dimensions
give every discriminant, and the fields of degree at least four give the
imaginary quadratic ones, so neither the base points of nontrivial
discriminant nor those of the quadratic fields carry weight in any minimal
derivation. What separates a base point from the family is propagation.
...
character, and in that form it is again one number:
\Cref{thm:p2primenumber} computes the rank that $\dim\Ext^{2}(E,E)$ must
reach, $(4+\rho)n^{2}-2n$ with $\rho$ the rank of a Hankel matrix of the
coefficients, and shows that an object reaching it satisfies the criterion.
What it cannot say is whether such an object exists.
