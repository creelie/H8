...
\texttt{frontier\_}\allowbreak\texttt{suffices} & it is in the closure once the three statements of \Cref{cor:fourremain} are adjoined, and once any of the pairs $\{\mathrm{L},\mathrm{M}\}$, $\{\mathrm{L},\mathrm{F3}\}$, $\{\mathrm{V},\mathrm{F3}\}$ of \Cref{thm:closure}(ii) is \\
\texttt{frontier\_}\allowbreak\texttt{minimal} & in each of the four sets every element is necessary: dropping any one leaves the conjecture underivable \\
\texttt{frontier\_}\allowbreak\texttt{smallest} & no single open statement suffices, and of the sixty-six pairs of open statements exactly those three do \\
