...
each of its seventy-eight theorems, that it depends on no axiom whatever, or on
propositional extensionality alone, which is what \texttt{decide} uses when it
reduces an equality of Booleans.\footnote{Sixty-nine of the seventy-eight report
\texttt{does not depend on any axioms}; the remaining nine report
...
dimension and the rank-zero obstruction, are not among the seventy-eight: the
...
\caption{The seventy-eight machine-checked theorems.}\label{tab:lean}\\
\toprule
...
\texttt{frontier\_}\allowbreak\texttt{suffices} & it is in the closure once (F3) alone is adjoined, which is \Cref{prop:f3ishc}, and once the pair $\{\mathrm{L},\mathrm{M}\}$ is; and, not minimally, once the three statements of \Cref{cor:fourremain} or either of $\{\mathrm{L},\mathrm{F3}\}$, $\{\mathrm{V},\mathrm{F3}\}$ is \\
\texttt{frontier\_}\allowbreak\texttt{minimal} & in each of the two minimal sets of \Cref{thm:closure}(ii) every element is necessary: dropping any one leaves the conjecture underivable \\
\texttt{frontier\_}\allowbreak\texttt{smallest} & of the twelve open statements exactly one, (F3), suffices alone, and of the sixty-six pairs exactly those containing (F3) and the pair $\{\mathrm{L},\mathrm{M}\}$ do \\
...
\texttt{secant\_route\_}\allowbreak\texttt{stops} & granting both open demands of the secant route reaches $\delta=\delta_{0}$ and neither $\delta\ne\delta_{0}$ nor the whole case \\
\texttt{propagation\_}\allowbreak\texttt{closes} & (P2) puts the whole imaginary quadratic case, and the Weil classes of every CM field, in the closure \\
\texttt{p2\_minimal\_}\allowbreak\texttt{count} & $\binom{4n}{2}-4n^{2}=2\binom{2n}{2}=2n(2n-1)$ for every $n\le60$, the dimension at which the criterion is met, \Cref{thm:p2numerical} \\
