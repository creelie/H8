...
\title[Base points and obstructions for Weil classes]
      {Explicit Base Points and Obstructions to Propagation for Weil Classes
...
\author{Priyabrata Mandal\textsuperscript{\ddag}}
...
\thanks{\textsuperscript{\dag}\,\textsuperscript{\ddag}Corresponding
authors: Deep Bhattacharjee\textsuperscript{\dag}, at
\texttt{d.bhattacharjee@erl-forschung.de} and \texttt{itsdeep@live.com}, and
Priyabrata Mandal\textsuperscript{\ddag}, at \texttt{priyabrata@manit.ac.in}.}
...
material was posted on Preprints.org, the preprint server of MDPI, on
5 August 2026, as
...
which is where the obstruction lives. The authorship has changed since that
MDPI preprint: Ushashi Bhattacharya, its second author, is the third author of
this paper, and Priyabrata Mandal has been added as the second and
...
the polarisation, that are Hodge only on the polarised family; for such a
complex we compute in closed form the dimension that $\Ext^{2}$ must reach,
and one complex reaching it suffices. By a descent lemma, the split families
of the CM fields of degree at least four, in infinitely many dimensions, give
every Weil family. Fourteen further routes are closed. This route needs three further statements. The
...
\input{sections/03_weiltype}
\input{sections/03b_explicit_weil}
\input{sections/04_characters}
\input{sections/05_fouriermukai}
\input{sections/06_divisor_obstruction}
\input{sections/07_numerology}
\input{sections/08_semiregularity}
\input{sections/09_hyperplane}
\input{sections/09b_kugasatake}
\input{sections/10_reduction}
\input{sections/10b_escape}
\input{sections/10c_construction}
\input{sections/10d_cmfields}
\input{sections/11_scope}
\input{sections/14_closure}
\input{sections/12_verification}
\input{sections/13_lean}

\appendix
\input{appendices/A_exterior}
\input{appendices/B_cdk}
\input{appendices/C_albert}
\input{appendices/D_scripts}
