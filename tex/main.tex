...
follows once a perfect complex with the Weil Chern character has $\Ext^{2}$ of minimal
dimension $2n(2n-1)$, which no complex supported in codimension $n$ attains.
Fourteen routes are closed. This route needs three further statements, and
the last of them, the conjecture for varieties that are not abelian, is
equivalent to the whole conjecture through $A\times\PP^{1}$; by
machine-checked computation the only other minimal route is the Lefschetz
standard conjecture with motivated classes, also equivalent to it. The Lefschetz standard conjecture holds
