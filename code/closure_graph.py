...
What is checked:
  ...
  (h) the rule set includes the routes of the literature through motivated
      ...
      the minimal sufficient sets are exactly two, {red_ab} and
      {lef_B, mot}; the sets {lef_B, red_ab}, {red_ab, vhc} and
      {P2, red_ab, red_weil} suffice and are not minimal; no minimal set uses
      anything proved here; and every member of a minimal sufficient set is
      itself a consequence of the Hodge conjecture, recorded with the result
      that proves it.  So each minimal set is not only sufficient: it is
  ...
  (i) the rule set contains the elementary implication red_ab => HC_ab of
      ...
      check (i) below removes the rule and confirms that the earlier answer
      is what the incomplete rule set gives, so that the change is traced to
      that one rule.
...
The point of (h) and (i) is the same one made about the whole paper.  The
two minimal routes to the conjecture are the conjecture for the varieties
that are not abelian, which is the conjecture itself, and the Lefschetz
standard conjecture with the motivatedness of Hodge classes, which together
are the conjecture; neither uses any statement proved here.
...
def run():
    ...
    check("the minimal sufficient sets are exactly two",
          sorted(sufficient) == expected,
          "{red_ab} and {lef_B, mot}"
    ...
    check("every member of a minimal sufficient set is a consequence of the "
          "conjecture, and P2 is in none of them",
          not_implied == [] and "P2" not in in_sets,
          "lef_B by thm:equivalence, mot by prop:lefmot, red_ab directly; "
          "no implication from the conjecture to P2 is known"
          if not_implied == [] and "P2" not in in_sets
          else str(not_implied))
    check("each minimal sufficient set is an equivalence with the conjecture",
          all(all(s in IMPLIED_BY_HC for s in S)
              and TARGET in closure(BASE | set(S)) for S in sufficient),
    ...
    check("every minimal sufficient set contains red_ab or mot",
          ...
          "conjecture for them, which is the whole conjecture, or through "
          "motivated classes")
    ...
    check("no minimal sufficient set uses a statement proved in this paper",
          not any(uses[S] for S in sufficient),
          "both rest on open statements and quoted theorems alone")
    ...
    full = RULES
    RULES = [r for r in full if r[2] != "prop:f3ishc"]
    try:
        was = []
        for k in range(0, len(LEAVES) + 1):
            for S in combinations(LEAVES, k):
        ...
        RULES = full
    check("without the rule of prop:f3ishc the search returns the four sets "
          "of the earlier version",
          sorted(was) == sorted([("lef_B", "mot"), ("lef_B", "red_ab"),
                                 ("red_ab", "vhc"),
                                 ("P2", "red_ab", "red_weil")]),
          "{lef_B, mot}, {lef_B, red_ab}, {red_ab, vhc}, {P2, red_ab, "
          "red_weil}: the change is that one rule and nothing else")
